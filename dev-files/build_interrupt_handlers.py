...
def generateSVChandler ():
  ...
  sFile += "@----------------------------------------- if calling task is background task, save its context\n"
  sFile += "  mrs   r12, psp\n"
  sFile += "  cbnz  r4, calling.task.is.not.background.task\n"
  sFile += "  ldr   r2, =var.background.task.context\n"
  sFile += "  str   r12, [r2]\n"
  sFile += "calling.task.is.not.background.task:\n"
  ...
  sFile += "  pop   {r4, lr}\n"
  sFile += "@----------------------------------------- Task context did change ?\n"
  sFile += "  cmp   r0, r1  @ R0:calling task context, R1:new task context\n"
  sFile += "  bne   context.change\n"
  sFile += "  cbz   r1, run.background.task\n"
  sFile += "  bx    lr  @ No change\n"
  sFile += "@----------------------------------------- Save context of preempted task\n"
  sFile += "context.change:\n"
  sFile += "  cbz   r0, perform.restore.context     @ Do not save context of background task\n"
  ...
  sFile += "run.background.task:\n"
  sFile += "  ldr   r0, =var.background.task.context\n"
  sFile += "  ldr   r0, [r0]\n"
  sFile += "  msr   psp, r0\n"
  sFile += "@--- Return from exception (background task: thread mode, process stack, no floating point)\n"
  sFile += "  ldr   r1, =0xFFFFFFFD\n"
  sFile += "  bx    r1\n\n"
  return sFile
